...
class LoginResponse {
  ...
  @override
  String toString() => 'LoginResponse[accessToken=$accessToken, role=$role, expiresIn=$expiresIn]';
  ...
  static LoginResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'access_token'), 'Required key "LoginResponse[access_token]" is missing from JSON.');
        assert(json[r'access_token'] != null, 'Required key "LoginResponse[access_token]" has a null value in JSON.');
        assert(json.containsKey(r'role'), 'Required key "LoginResponse[role]" is missing from JSON.');
        assert(json[r'role'] != null, 'Required key "LoginResponse[role]" has a null value in JSON.');
        assert(json.containsKey(r'expires_in'), 'Required key "LoginResponse[expires_in]" is missing from JSON.');
        assert(json[r'expires_in'] != null, 'Required key "LoginResponse[expires_in]" has a null value in JSON.');
        return true;
      }());
